...
using static LoDCompanion.Code.BackEnd.Services.Player.EstateFurnishing;
using static LoDCompanion.Code.BackEnd.Services.Player.ServiceLocation;
...
namespace LoDCompanion.Code.BackEnd.Services.Player
{
    ...
    public class Arena : ServiceLocation
    {
        ...
        private int GetArenaModifier(Hero hero)
        {
            ...
            switch (Bout)
            {
                case ArenaBout.Group: modifier -= 10; break;
                case ArenaBout.SemiFinal: modifier -= 15; break;
                case ArenaBout.Final: modifier -= 20; break;
            }
            return modifier;
        }
    }
    ...
    public class Shrine : EstateFurnishing
    {
        ...
        {
            var choiceResult = await userRequest.RequestChoiceAsync("Choose a god to devote this shrine to.", Enum.GetNames<GodName>().ToList(), name => name);
            await Task.Yield();
